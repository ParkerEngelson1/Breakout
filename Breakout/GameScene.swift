...
//  Breakout
//
//  Created by Parker Engelson on 3/30/26.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func makeBricks() // first, remove any leftover bricks (from prior game)
    {
        for brick in bricks {
            if brick.parent != nil {
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if abs(ball.physicsBody!.velocity.dy) < 100 {
            // ball has stalled in the y direction, so kick it randomly vertical
